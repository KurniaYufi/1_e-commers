            ...
            {HeroData.map((item) => (
              <div key={item.id} className=''>
                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                  {/* Text Section */}
                  <div 
                    className='flex flex-col justify-center gap-4 sm:pl-3 pt-12 sm:pt-0 text-center md:text-left order-2 md:order-1 relative z-10'
                    data-aos="zoom-in"
                  >
                    ...
                    <h1 className='text-5xl uppercase text-white md:text-[70px] xl:text-[150px] font-bold whitespace-nowrap'>{item.title2}</h1>
                    <div>
                      <button className='bg-primary text-white cursor-pointer hover:scale-105 duration-300 py-2 px-8 mb-4 rounded-full relative z-10'>
                        Shop By Category
                      </button>
                    </div>
                  </div>
                  ...
                  <div className='order-1 md:order-2'>
                    <div data-aos="zoom-out" className="relative z-40">
                      <img 
                        src={item.img} 
                        alt={`Hero Image ${item.id}`} 
                        className='w-[300px] h-[300px] md:h-[450px] lg:scale-140 object-contain 
                        mx-auto drop-shadow-[-8px_4px_6px_rgba(0,0,0,0.2)]' 
                      />
                    </div>
                  </div>
                </div>
